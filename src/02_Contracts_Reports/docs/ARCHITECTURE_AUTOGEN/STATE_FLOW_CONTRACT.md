...
| Runtime verb interpreter | `src/logic/runtime/runtime-verb-interpreter.ts` | `window "action"` → `handleAction` | **Intent name without prefix** | Dispatches `params.name.replace("state:", "")` (e.g. `currentView`). **Mismatch:** state-resolver expects `state:currentView`; this path does not update derived state for view. |
