# Blueprint → Runtime Wiring

**Source:** Logic Plan 3 — [3_BLUEPRINT_TO_RUNTIME_WIRING_PLAN.md](../../cursor/logic/complete/3_BLUEPRINT_TO_RUNTIME_WIRING_PLAN.md).  
**Classification:** FOUNDATIONAL — Compilation vs runtime boundary; primary architecture reference: src/docs/ARCHITECTURE_AUTOGEN, src/docs/SYSTEM_MAP_AUTOGEN

How Blueprint JSON becomes runtime behavior across Layout and Logic. Where compilation stops and runtime begins; which fields are static vs dynamic; rules preventing blueprint from hardcoding layout fallbacks.
...
**Screen JSON** is the loaded document (e.g. app.json) whose root is `json?.root ?? json?.screen ?? json?.node` (see BLUEPRINT_RUNTIME_INTERFACE.generated.md).
...
## Verification (Plan 3 completion)

- **Compiler:** `src/scripts/blueprint.ts` does not reference layoutId/defaultLayout; BLUEPRINT_RUNTIME_INTERFACE states "No layout ids written by blueprint for sections".
- **getDefaultSectionLayoutId:** `src/layout/page/page-layout-resolver.ts` returns undefined when templateId missing or template has no defaultLayout; no code fallback.
- **Runtime strips section layout from params:** applyProfileToNode strips moleculeLayout, layoutPreset, layout, etc.; section layout comes from override → explicit → template default only.
- **Template JSON:** templates.json holds per-role and defaultLayout only; no layout IDs in blueprint script.
