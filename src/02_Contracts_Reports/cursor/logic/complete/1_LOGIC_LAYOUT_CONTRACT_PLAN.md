# 1 — Logic ↔ Layout Contract Plan

**Execution order:** 1 of 6 (foundational)  
...
- **Contract published:** [src/docs/ARCHITECTURE_AUTOGEN/LOGIC_LAYOUT_CONTRACT.md](../../docs/ARCHITECTURE_AUTOGEN/LOGIC_LAYOUT_CONTRACT.md) is the canonical reference.
- **No Logic → Layout store writes:** Only `app/layout.tsx` and `layout-dropdown.tsx` call `setLayout` (UI). No logic/behavior engine calls layout store setters.
- **Single resolver:** `applyProfileToNode` in `src/engine/core/json-renderer.tsx` assigns section layout ID; precedence: override → explicit node.layout → template default → undefined.
