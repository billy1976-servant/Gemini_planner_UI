# 6 — User Preference Adaptation Plan

**Execution order:** 6 of 6 (engine: signals → trait weights for Plan 5)  
...
- **Trait registry:** Same layout ID → traits JSON as Layout Decision Engine (Plan 5). Add before this plan's implementation.
- **Preference memory:** Define trait id → number shape and persistence strategy (session, user profile, etc.) before the Decision Engine consumes preference weights.

---

## System Purpose

Capture **"more like this"** and **"less like this"** signals from the user and persist **preference weights per trait** so that the Layout Decision Engine (Plan 5) can bias scoring toward preferred layouts when multiple compatible options exist. The system does not silently override anything: it does not mutate the layout store or node.layout. It only reads current layout state to translate signals into trait deltas and writes to a separate preference memory (trait weights). Layout resolution order remains **override > explicit > suggestion > default**.
...
| **Trait registry** | Same as Layout Decision Engine (Plan 5): layout ID → set of trait identifiers. Used only to look up "which traits does this layout have?" when applying a signal. |
...
2. Look up the set of trait identifiers for each of those layout IDs from the **trait registry** (same JSON as used by the Layout Decision Engine, Plan 5).
...
- **Layout engine usage:** The layout engine (and Layout Decision Engine, Plan 5) only **read** these weights. They never write them. The preference adaptation layer is the only writer. So from the layout engine's perspective, preference memory is read-only.
...
| Trait registry | Same JSON as Layout Decision Engine (Plan 5) (layout ID → traits). |
...
The Decision Engine accepts optional **preference weights** (trait id → number). It uses them when scoring compatible layouts. This plan defines how those weights are produced and updated; it does not implement the scoring (that is in Plan 5).

### Trait registry

The same trait registry used by the Layout Decision Engine (Plan 5) is used here to map "current layout ID at signal time" → traits. No duplicate registry; single source of truth in JSON.
...
- **Spec published:** [src/docs/ARCHITECTURE_AUTOGEN/USER_PREFERENCE_ADAPTATION.md](../../docs/ARCHITECTURE_AUTOGEN/USER_PREFERENCE_ADAPTATION.md) is the canonical reference.
- **No existing preference adaptation:** No runtime code implements "more like this" / "less like this" or trait-weight persistence.
- **Layout store:** Only UI writes layout state; preference layer will only read at signal time and write to separate preference memory.
