...
import { learningEngine, learningPresentation } from "../engines/learning.engine";
import { calculatorEngine, calculatorPresentation } from "../engines/calculator/calculator.engine";
import { abcEngine, abcPresentation } from "../engines/abc.engine";
import { decisionEngine, decisionPresentation } from "../engines/decision/decision.engine";
import { summaryEngine, summaryPresentation } from "../engines/summary/summary.engine";
...
export function applyEngine(flow: EducationFlow, engineId: EngineId): EngineFlow {
  ...
  try {
    ...
    console.log(`[EngineRegistry] Applying execution engine: ${engineId}`, {
      engineFunction: engine.name || 'anonymous',
      flowId: flow.id,
      stepCount: flow.steps.length,
      sourceFile: engineId === 'learning' ? 'learning.engine.ts' :
                   engineId === 'calculator' ? 'calculator/calculator.engine.ts' :
                   engineId === 'abc' ? 'abc.engine.ts' : 'unknown'
    });
    const result = engine(flow);
    console.log(`[EngineRegistry] Engine ${engineId} transformation complete`, {
      originalSteps: flow.steps.length,
      transformedSteps: result.steps.length,
      stepOrderChanged: flow.steps.map(s => s.id).join(',') !== result.steps.map(s => s.id).join(',')
    });
    return result;
  } catch (error) {
