...
### Explicit single-authority audit list

Run verification (grep or script); exactly one module may export each of the following:

| Symbol / concern | Single allowed exporter | Verification |
|------------------|-------------------------|---------------|
| `deriveState` | state/state-resolver.ts | No other file exports deriveState. |
| `getSectionLayoutId` | layout/ (section-layout-id.ts or layout index) | No other file exports getSectionLayoutId. |
| `resolveLayout` | layout/ (resolver or layout index) | No other file exports resolveLayout for section layout definition. |
| Component registry map (type → React component) | engine/core/registry.tsx | No other file exports the same Registry map. |

**Contracts:** All contracts must live ONLY in `/contracts`. If `system/contracts` exists, it must be re-export-only from `contracts/` or removed. No duplicate SystemContract or contract sprawl outside contracts/.
...
- docs: system-architecture, refactor_ROUND 3/architecture, RUNTIME_AUTHORITY_LADDER, TRUNK_ARCHITECTURE_TARGET (Section 0 hard rules)

---

*Planning only; execution later. Hard rules: see TRUNK_ARCHITECTURE_TARGET.md Section 0.*
