...
export default function IntegrationFlowEngine() {
  ...
  // Load education flow and generate presentation model for EducationCard
  useEffect(() => {
    const educationCard = CARD_REGISTRY.find(card => card.id === "education");
    if (!educationCard) return;
    
    // Default flow ID for education card
    const flowId = "test-flow";
    
    loadFlow(flowId)
      .then((loadedFlow) => {
        // Get engine ID from engine state or default to "learning"
        const engineId = engineState?.education?.engineId || "learning";
      ...
      .catch((err) => {
        console.error("[IntegrationFlowEngine] Failed to load flow for presentation:", err);
      });
  }, [engineState]);
            ...
            return (
              ...
              >
                ...
                <CardComponent
                  ...
                  {...(controller.id === "education" && presentationModel ? { presentation: presentationModel } : {})}
                />
              </div>
            );
          })}
