# State and Override Orchestration Plan

**Classification:** FOUNDATIONAL — Override precedence and storage; primary architecture reference: docs/SYSTEM_MASTER/
...
3. **Logic suggestion** — recommended layout ID from Decision Engine (when integrated).
...
| Item | Storage |
|------|---------|
| **User overrides** | In-memory override maps passed into renderNode/applyProfileToNode (and/or a dedicated override store if introduced). Not in layout-store's template/experience state. |
