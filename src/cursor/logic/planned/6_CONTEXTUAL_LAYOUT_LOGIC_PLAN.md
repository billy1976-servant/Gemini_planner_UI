# Contextual Layout Logic Plan

**Classification:** REFERENCE — Content→traits logic; primary architecture reference: docs/SYSTEM_MASTER/
...
- **Output:** Either a list of trait identifiers (suggested traits) or a map of trait id → weight delta (contextual trait weights). The Layout Decision Engine will use this to score compatible layouts.
...
- **Output** is only trait identifiers or trait weights. Layout IDs are never mentioned. The Layout Decision Engine is responsible for mapping traits to layout IDs via the trait registry.
...
- The **Layout Decision Engine** takes: (1) the compatible layout ID set from the compatibility engine, and (2) the suggested traits or contextual weights from this plan's engine. It scores compatible IDs by traits and weights and returns a recommended layout ID. So: compatibility filters; contextual logic suggests traits; decision engine ranks. No bypassing of compatibility.
...
The trait registry (shared with Decision Engine and User Preference Adaptation) defines which traits exist. Contextual rules only reference trait identifiers that appear in that registry. They do not define new layout IDs.
