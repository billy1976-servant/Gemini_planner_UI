# 3 — Blueprint → Runtime Wiring Plan

**Execution order:** 3 of 6 (foundational)  
...
**Screen JSON** in this doc is the loaded document (e.g. app.json) whose root is `json?.root ?? json?.screen ?? json?.node` (see BLUEPRINT_RUNTIME_INTERFACE.generated.md).
...
- **Contract published:** [src/docs/ARCHITECTURE_AUTOGEN/BLUEPRINT_TO_RUNTIME_WIRING.md](../../docs/ARCHITECTURE_AUTOGEN/BLUEPRINT_TO_RUNTIME_WIRING.md) is the canonical reference.
- **Compiler:** blueprint.ts does not emit layout IDs; BLUEPRINT_RUNTIME_INTERFACE: "No layout ids written by blueprint for sections".
- **getDefaultSectionLayoutId:** Returns undefined when template missing or no defaultLayout; no code fallback.
- **Runtime:** Section layout from override → explicit → template default only; params stripped in applyProfileToNode.
