 ...
 */
export function persistContractArtifact(result: LayoutContractResult): string | null {
  ...
  try {
    ...
    const screenKey = (result.debugDump?.screenKey ?? "unknown").replace(/[^a-zA-Z0-9-_]/g, "_");
    const dir = path.join(process.cwd(), "artifacts", "pipeline-contract");
    if (!fs.existsSync(dir)) {
      fs.mkdirSync(dir, { recursive: true });
    }
    const filename = `${date}_${time}_${screenKey}.json`;
