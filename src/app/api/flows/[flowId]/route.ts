...
const O1_APP_BASE = path.resolve(process.cwd(), "src", "01_App");
...
/** Find a flow JSON under 01_App by flowId (filename stem or json.id). */
function findFlowInO1App(dir: string, flowId: string): string | null {
  ...
  for (const entry of entries) {
    ...
    if (entry.isFile() && entry.name.endsWith(".json")) {
      ...
      if (stem === flowId) return fullPath;
      try {
        const content = fs.readFileSync(fullPath, "utf8");
        const json = JSON.parse(content);
        if (json?.id === flowId) return fullPath;
      } catch {
        /* skip */
      }
    } else if (entry.isDirectory() && !entry.name.startsWith(".")) {
      const found = findFlowInO1App(fullPath, flowId);
...
export async function GET(
  ...
  try {
    ...
    if (!fs.existsSync(jsonPath)) {
      ...
      if (foundPath) {
        jsonPath = foundPath;
      } else if (fs.existsSync(O1_APP_BASE)) {
        const businessRoot = path.join(O1_APP_BASE, "Business");
        const christianRoot = path.join(O1_APP_BASE, "Christian");
        const o1Found =
          findFlowInO1App(businessRoot, flowId) ?? findFlowInO1App(christianRoot, flowId);
        if (o1Found) jsonPath = o1Found;
        else return NextResponse.json({ error: `Flow not found: ${flowId}` }, { status: 404 });
      } else {
