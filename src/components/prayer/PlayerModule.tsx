...
export function PlayerModule({
  ...
  const shouldLoop = replayPrayer?.source === "live_room";

  const handleListeningFlush = async (seconds: number) => {
    if (!Number.isFinite(seconds) || seconds <= 0) return;
    try {
      await fetch("/api/prayer/progress", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="prayer-player-module" data-module="player">
      <PrayerPlayer
        ...
        useVideo={useVideo}
        onListeningSecondsFlush={handleListeningFlush}
      />
