...
1. User presses play → POST played (increment total), POST join (get sessionId). Store sessionId in state.
2. While playing, every 20s → POST heartbeat with prayerId + sessionId.
3. User stops or leaves page → POST leave with sessionId (use beforeunload or visibilitychange + sendBeacon if needed).
4. Metrics: totalListeners from current prayer (after refetch or from played response). Live count from GET listeners?prayerId= (poll every 15–20s when prayer is loaded). Prayer length from audio duration (reported by player to parent).
