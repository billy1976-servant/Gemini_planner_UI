...
function renderContentBlocks(content: ContentBlock[], options?: RenderContentBlocksOptions) {
  ...
  return content.map((block, i) => {
    ...
    if (block.type === "paragraph") {
      ...
      if (editorOpts?.isEditor && editorOpts.screenId && options?.onParagraphChange) {
        return (
          <InlineEditableText
            key={i}
            value={block.text}
...
export default function GospelDiscipleship() {
  ...
  function renderScreen(screen: Screen) {
    ...
    const contentBlocksOpts = isEditor ? { isEditor: true, screenId: screen.id, onParagraphChange: (idx: number, t: string) => updateScreenContentBlock(screen.id, idx, t) } : undefined;
