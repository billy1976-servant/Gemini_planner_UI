# Prayer/Bible Study Live Collaboration — Build Plan
...
| **Files to create** | `room/ModeratorPanel.tsx` — collapsible sidebar/drawer; optional: `room/useRoomRecording.ts` hook that wraps PrayerRoomRecorder + mixedStream + publish. |
...
1. **Phase A** — Recording + moderator panel (implement first).
2. **Phase B** — Screen sharing (after A).
3. **Phase C** — Annotations (after B).
4. **Phase D** — Saved boards (after C).
5. **Phase E** — Replay timeline (after D).
