...
- When only participant video is available (no deck/screen share/saved slides), a **VideoContentView** shows the host’s camera or first remote video so annotations can overlay it.
...
- **StudyPage** (in `StudyPagesManager`) now supports:
  - `imageUrl?: string` — data URL of the captured slide/screen/video frame.
  - `annotations?: AnnotationStroke[]` — strokes on that slide at save time (for replay).
...
- **SessionReplayViewer** accepts optional `sessionStudyPages?: StudyPage[]`. When there is no deck (or for extra slides), it picks the current “session slide” by time (`timestampSec <= currentTimeSec`) and shows that slide’s `imageUrl` with annotation timeline replay.
...
- **AnnotationOverlay.tsx**: Optional tools **Rectangle**, **Arrow**, **Text** — left as TODO; implement when needed.
