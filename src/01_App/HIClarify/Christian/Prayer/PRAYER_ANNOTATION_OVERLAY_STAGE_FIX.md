# Prayer Annotation Overlay Stage Fix — Report
...
- **.prayer-content-stage**:
  - `position: relative` — establishes the containing block for the overlay’s `position: absolute`
  - `width: 100%` — stage spans the available width
  - `min-height: 200px` — stage has a minimum height so the overlay has a drawable area even with minimal content
...
- **Single wrapper** (the element with `ref={containerRef}`):
  ...
  - `zIndex: 50` — above the content (default stacking)
  - `pointerEvents: editable ? "auto" : "none"` — canvas receives pointer events only when editable
- **Canvas**:
  - `position: "absolute"`, `inset: 0`, `width: "100%"`, `height: "100%"`
  - `zIndex: 51` — above the wrapper’s background, below the toolbar
  - `touchAction: "none"` — avoids touch scrolling stealing drag
  - `background: "transparent"` — no tint over the content
  - `pointerEvents: editable ? "auto" : "none"` — same as wrapper for clarity
...
- **Single stage:** Content and overlay are siblings inside `.prayer-content-stage`, so the overlay’s `position: absolute; inset: 0` covers exactly the content area.
...
- **Toolbar:** Floats at bottom center over the content and does not push the layout; host can draw on the full content area including under the toolbar’s horizontal strip.

**Verification:** Open room as host → turn annotations on → confirm overlay and toolbar sit over the current view (video/slide/screen share) → draw on the content → strokes appear on the content and console logs `ANNOTATION POINTER DOWN`.
