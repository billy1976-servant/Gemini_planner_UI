...
export interface PrayerRoomParticipantsProps {
  ...
  /** When provided, host can invite listeners to speak. */
  onInviteToSpeak?: (participantId: string) => void;
      ...
      <ul className="prayer-room-participants-list" style={{ listStyle: "none", margin: 0, padding: 0 }}>
        {participants.map((p) => (
          ...
          >
            ...
            {canInviteToSpeak && onInviteToSpeak && p.role === "listener" && (
              <button
                type="button"
                onClick={() => onInviteToSpeak(p.participantId)}
                style={{
                  marginLeft: "0.5rem",
