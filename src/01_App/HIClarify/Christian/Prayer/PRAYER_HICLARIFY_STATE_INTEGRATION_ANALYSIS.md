...
| **State resolver** | [src/03_Runtime/state/state-resolver.ts](src/03_Runtime/state/state-resolver.ts) | Pure `deriveState(log)` → `DerivedState`. Single source of truth for derived snapshot. |
...
- **currentView** — active screen/path (e.g. `state:currentView`).
- **values** — `Record<string, any>`. Generic key/value surface used for domain slices (planner structure, palette, list confirm id, etc.).
- **layoutByScreen** — section/card/organ overrides and nav targets per screen.
- **dashboardLayout** — widget rects per screen (dedicated intents `dashboard.layout` / `dashboard.updateWidget`).
- **journal**, **scans**, **interactions** — append-only / domain-specific.

No change to the resolver is required to add Prayer: **`state.update`** already supports any key in `values`.

### 1.3 Patterns already used for “state separate from TSX”
...
- **getSlice()**: `getState()?.values?.[STRUCTURE_KEY]` → full slice (items, tree, calendarView, selectedDate, parserStaging, etc.).
- **writeSlice(next)**: `dispatchState("state.update", { key: STRUCTURE_KEY, value: next })`.
- All structure mutations go through actions that read slice → mutate → write slice. UI and other domains do not duplicate this state.
...
- `setPalette(name)` → `dispatchState("state.update", { key: "paletteName", value: next })`.
- `getPaletteName()` → `getState()?.values?.paletteName ?? "default"`.
...
- “SessionEngine can use the **same store** for ‘current session id’, ‘recording on’, ‘screen share on’, etc., so JSON and TSX screens stay in sync.”
- “Domain-specific session data … can remain in a dedicated store (or API) and be **referenced by state keys** (e.g. `state.values.sessionId`).”
...
Room/connection state (room, participantId, role, hostId, liveKitToken, etc.) and WebRTC/recording (usePrayerRoomWebRTC, useRoomRecording) remain in PrayerRoom; they are not in scope for this “state layer only” integration.

### 2.3 Duplication and “fighting” today
...
- **Session-slide index** is adjusted in a useEffect when `currentSessionSlideIndex >= sessionSlidesWithImages.length` and is also set inside a `setStudyPages` callback to avoid stale closure — two different patterns for one concept.
...
|---------------------|--------------------|---------------------------|
| **Active slide** (which content + index) | “Current entity” / active item | Single derived value from slice: e.g. `contentSource` + `currentSlideIndex` + `currentSessionSlideIndex` in one slice |
...
### 4.1 Reuse existing pieces — no new architecture
...
| **Annotation sync** | handleAnnotationChange depends on many useState deps; slideId can lag | slideId from slice; one source of truth for “current slide” when recording a stroke |

Rough quantification:

- **Fewer props**: ~15–20 fewer if ModeratorPanel and content components read from state.
- **Fewer useState**: 6–8 removed from PrayerRoom (studyPages, annotationStrokes, annotationVisible, activeDeck, currentSlideIndex, currentSessionSlideIndex; optionally exported timelines if you move them).
- **Fewer duplicated branches**: The “which content / what slideId” logic goes from 3 to 1.
- **Fewer race-prone patterns**: No setState-in-callback for index sync; one write for “new session slide + switch to it”.
...
- **Phase 1**: Add `prayerRoomSession` and getSlice/writeSlice. PrayerRoom **writes** all session/slide/annotation updates to the slice (writeSlice) and **reads** from the slice (getSlice(roomId) from state). Remove the matching useState in PrayerRoom so the slice is the only source for that data. Keep passing props to children so child components don’t need to change yet.
...
2. **In PrayerRoom**:
   ...
   - Replace the 6–8 session/slide/annotation useState variables with: read from `getSlice(roomId)` (or from stateSnapshot), and write via `writeSlice(roomId, { ...slice, ... })`.
...
- **Prayer currently does not use it**: all session/slide/annotation state is local state and props in PrayerRoom, which causes duplicated “active slide” logic and brittle ordering.
...
- **Recommended path**: Option C — Phase 1: slice + PrayerRoom reads/writes slice only (remove matching useState); Phase 2: children subscribe and read slice to reduce props.
