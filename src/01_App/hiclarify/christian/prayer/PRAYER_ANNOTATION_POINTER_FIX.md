# Prayer Annotation Pointer Fix ΓÇö Report
...
- No parent in the Prayer room content block has a higher zIndex that would cover the overlay; the content wrapper uses `zIndex: 101` only for the host/annotation-visible case, and the overlay is a child of that wrapper, so the overlayΓÇÖs zIndex 20 places it above the media content inside the wrapper.
...
**Verification steps:** Open room as host ΓåÆ turn annotations on ΓåÆ see toolbar and (slightly red-tinted) canvas ΓåÆ click/drag on canvas ΓåÆ see "ANNOTATION POINTER DOWN" in console and drawn lines on the canvas.
