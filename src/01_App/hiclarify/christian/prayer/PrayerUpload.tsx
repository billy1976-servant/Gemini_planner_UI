...
export function PrayerUpload({ onUploaded, groupId }: PrayerUploadProps) {
  ...
  const getStream = useCallback(async (type: RecordingType): Promise<MediaStream | null> => {
    try {
      if (type === "audio") return await navigator.mediaDevices.getUserMedia({ audio: true });
      if (type === "video") return await navigator.mediaDevices.getUserMedia({ video: true, audio: true });
      if (type === "screen") return await navigator.mediaDevices.getDisplayMedia({ video: true, audio: true });
      return null;
    } catch {
      return null;
    }
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (recordedBlob) {
          formData.append("audio", recordedBlob, recordingType === "audio" ? "recording.webm" : "recording.webm");
  ...
  return (
    <div className="prayer-card" style={{ maxWidth: "420px" }}>
      ...
      <form className="prayer-upload-form" onSubmit={handleSubmit}>
        ...
        <textarea id="prayer-text" value={prayerText} onChange={(e) => setPrayerText(e.target.value)} placeholder="Full prayer textΓÇª" />

        <label style={{ display: "block", marginTop: "0.75rem" }}>Record: Audio only, Prayer (video), or Screen</label>
        <div style={{ display: "flex", flexWrap: "wrap", gap: "0.35rem", alignItems: "center", marginBottom: "0.5rem" }}>
          {(["audio", "video", "screen"] as const).map((type) => (
            <button
              key={type}
              type="button"
              onClick={() => recordingState === "idle" && setRecordingType(type)}
              disabled={recordingState === "recording"}
              style={{
                padding: "0.4rem 0.75rem",
                borderRadius: 8,
                border: "1px solid var(--prayer-card-border)",
                background: recordingType === type ? "var(--prayer-play-bg)" : "transparent",
                color: recordingType === type ? "#fff" : "var(--prayer-text)",
                cursor: recordingState === "recording" ? "not-allowed" : "pointer",
                fontSize: "0.8125rem",
              }}
            >
              {type === "audio" ? "Audio only" : type === "video" ? "Prayer (video)" : "Screen"}
            </button>
          ))}
        </div>
          ...
          {recordingState === "recording" && (
            <>
              <span style={{ fontSize: "0.875rem", color: "var(--prayer-text-muted)" }}>
                RecordingΓÇª {Math.floor(recordDuration / 60)}:{String(recordDuration % 60).padStart(2, "0")}
              </span>
        ...
        {recordedBlob && (
          <div style={{ marginBottom: "1rem" }}>
            {recordingType === "audio" ? (
              ...
              <video src={URL.createObjectURL(recordedBlob)} controls style={{ width: "100%", maxHeight: 200, borderRadius: 8 }} />
            )}
          </div>
        )}
        ...
        <button type="submit" disabled={loading || !hasAudio || !title.trim()}>
          {loading ? "PublishingΓÇª" : "Publish prayer"}
        </button>
      </form>
    </div>
  );
