...
2. **OAuth flow:** Click "Sign in with Google" → `signIn("google", { callbackUrl: "/prayer" })` → NextAuth redirects to Google → Google redirects to **NEXTAUTH_URL + `/api/auth/callback/google`**. If `NEXTAUTH_URL` is wrong or that URL is not in Google Console’s Authorized redirect URIs, the callback fails or the app crashes.
...
4. **Why it looked like “login”:** The Prayer page runs `getGroups()` on mount. If you click “Login with Google” and then the redirect lands back on `/prayer`, the page remounts and runs `getGroups()` again. If that fetch threw (e.g. old bundle), the error overlay appeared and felt like “login failed.”
...
|----------|----------|----------------|
| `GOOGLE_CLIENT_ID` | Yes | From Google Cloud Console → Credentials → OAuth 2.0 Client ID |
...
In **Google Cloud Console** → **APIs & Services** → **Credentials** → your **OAuth 2.0 Client ID** → **Authorized redirect URIs**, add **exactly** one of these (must match the origin you use in the browser and `NEXTAUTH_URL`):
...
5. **Test:** Open the app at the same URL as `NEXTAUTH_URL`, click “Sign in with Google”. After redirect, the app should load without a “Failed to fetch” crash; if the groups API is down, you get an empty list and a console `[safeFetch]` warning.
