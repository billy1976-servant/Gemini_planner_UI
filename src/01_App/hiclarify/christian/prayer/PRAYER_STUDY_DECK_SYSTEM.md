...
- **PrayerRoom.tsx**
  ...
  - Content area: if `activeDeck` and no screen share ΓåÆ `SlideViewer` + `AnnotationOverlay`; if screen share ΓåÆ `ScreenShareView` + `AnnotationOverlay`. Annotation overlay stays on top of slide or screen share.
...
- **ModeratorPanel.tsx**
  - STUDY SLIDES: ΓÇ£Open deckΓÇ¥ toggles `StudyDeckManager`; when a deck is selected, Next / Previous / jump-to-slide (dropdown); deck selection via `onSelectDeck`, `activeDeck`, `currentSlideIndex`, `onNextSlide`, `onPrevSlide`, `onJumpSlide`.
  - ANNOTATION: unchanged (draw / highlight / erase, show/hide, clear).
  - Recording: ΓÇ£Export session (recording + JSON)ΓÇ¥ when `canExportSession`; below that, `SessionReplayViewer` when there is a recorded blob and timelines.
  - Replay uses `recordedBlob`, `sessionTimeline`, `annotationTimeline`, `activeDeck` (deck used in session).

- **StudyPagesManager.tsx**
  - Unchanged. Existing ΓÇ£Save pageΓÇ¥ / snapshot behavior is preserved. No removal of study page tools.

- **AnnotationOverlay.tsx**
  - Unchanged for this feature. Strokes still have optional `mode` (draw/highlight/erase). Recording is done in PrayerRoomΓÇÖs `handleAnnotationChange` by pushing to `annotation-timeline`.
...
1. **Recording start**  
   Host starts recording ΓåÆ `initSessionTimeline(startSec)` and `initAnnotationTimeline(startSec)` with wall-clock start time. Recording runs as today (mixed stream + optional screen share).

2. **Slide changes**  
   Host changes slide ΓåÆ `recordSlideChange(slideId, index)`. Event timestamp is `nowSec()` = seconds since recording start. Session timeline is used in replay to decide which slide to show at a given playback time.

3. **Annotations**  
   Host draws ΓåÆ on stroke complete, `handleAnnotationChange` pushes to `annotation-timeline` with `slideId` (current slide), `timestampSec: recording.recordDurationSec`, and path/type/color/width. Replay uses annotation timeline to draw strokes up to current playback time for the current slide.

4. **Replay**  
   `SessionReplayViewer` uses the recordingΓÇÖs `currentTime` as ΓÇ£seconds since start.ΓÇ¥ It:
...
- **StudyPagesManager** and existing ΓÇ£Save pageΓÇ¥ / snapshot tools are unchanged.
...
```
Host selects deck ΓåÆ activeDeck, currentSlideIndex
Host starts recording ΓåÆ init session + annotation timelines
Host changes slide ΓåÆ recordSlideChange(slideId, index)
Host draws ΓåÆ recordAnnotationEvent({ slideId, timestampSec, type, path, color, width })
Host stops recording ΓåÆ blob + timelines available for export/replay
Export ΓåÆ JSON manifest + recording file
Replay ΓåÆ SessionReplayViewer(blob, sessionTimeline, annotationTimeline, deck) ΓåÆ play/pause/seek, slide + annotations by time
```
