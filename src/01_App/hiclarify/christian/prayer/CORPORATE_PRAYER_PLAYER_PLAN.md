# Corporate Prayer Player ΓÇö Implementation Plan
...
- PrayerWaveform: accept `progress` (0ΓÇô1) and `duration`; bars span 100% width; fill from left up to `progress`.
...
**Target:** When audio ends ΓåÆ restart from beginning immediately. No delay. Continuous.
...
**Target:** Keep ΓêÆ15s and +15s. Ensure waveform and countdown update correctly after skip.
...
**Implementation:**
- PrayerLibrary already has secondary mode with expandable list. Change copy: "More prayers" ΓåÆ "Past prayers", "Hide other prayers" ΓåÆ "Hide past prayers". Same behavior.
...
**Implementation:**
- **CSS:** Add keyframes for a slow-shifting gradient (e.g. hue or position shift over 15ΓÇô20s). Apply to `.prayer-platform` background.
