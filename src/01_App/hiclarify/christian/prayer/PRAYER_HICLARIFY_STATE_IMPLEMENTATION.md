...
### Phase 1 ΓÇö State slice integration
...
### Phase 2 ΓÇö Remove prop drilling
...
Manual verification: open a room, join as host, select a deck, change slides, draw annotations, save page, use ΓÇ£Annotate screen,ΓÇ¥ clear/undo annotations. All behavior should match pre-integration, with state now in `state.values.prayerRoomSession`.
...
4. **Replay / SessionReplayViewer**  
   No changes were made to SessionReplayViewer or replay data flow; it continues to receive timelines and deck/pages from parent. Any future ΓÇ£replay stateΓÇ¥ could also live under the same or a related state key if desired.
