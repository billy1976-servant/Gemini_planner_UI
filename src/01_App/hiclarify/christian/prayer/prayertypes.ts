 ...
 * All code lives under Gospel/Prayer ΓÇö no dependencies on structure engine or resolver.
 */
...
export interface Prayer {
  ...
  participants?: string[];
  /** Optional saved study pages attached to a live recording replay (metadata; may include imageUrl and annotations). */
  studyPages?: {
    id: string;
    title: string;
    createdAt: string;
    timestampSec?: number;
    imageUrl?: string;
    annotations?: { id: string; points: { x: number; y: number }[]; mode?: string }[];
  }[];
}
