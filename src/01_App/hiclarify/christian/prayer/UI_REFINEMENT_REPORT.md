# Prayer Player UI Refinement ΓÇö Report
...
| File | Changes |
|------|--------|
| **PrayerPlayer.tsx** | Reorganized into horizontal layout: main row `[ Play button \| Waveform wrap \| Time remaining ]`, then control row with ΓêÆ15s, +15s, and a single **Speed** button that cycles 1x ΓåÆ 1.25x ΓåÆ 1.5x. Waveform area is click-to-seek. Removed separate progress bar and stacked layout. |
...
- **Layout:** Single horizontal player row (play \| full-width waveform \| time remaining) plus a minimal control row (ΓêÆ15s, +15s, Speed).
