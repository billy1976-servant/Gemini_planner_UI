...
import { getValueDimension } from "./value-dimensions";
import type { IndustryModelId, AssumptionVariable } from "../../value/assumption-library";
...
export function translateValue(input: ValueTranslationInput): ValueTranslationOutput {
  ...
  if (!industryModelData) {
    // If no industry model is found, we can't compute value blocks.
    // Return a fully shaped, but empty, ValueTranslationOutput.
    return {
      valueImpactBlocks: [],
      rankedValueConclusions: [],
      appliedAssumptions: [],
      appliedResearchFacts: [],
